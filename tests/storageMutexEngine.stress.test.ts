...
describe('StorageMutexEngine Stress, High Contention & Zero-Deadlock Benchmarks', () => {
  ...
  describe('Suite 1: 1,000 Concurrent Lock Requests', () => {
    it('test_01_1: processes 1,000 concurrent lock requests across 50 keys without deadlocks or unhandled rejections', async () => {
      const numEngines = 10;
      ...
      for (let i = 0; i < numEngines; i++) {
        ...
        for (let j = 0; j < requestsPerEngine; j++) {
          const key = `key_${(i + j) % numKeys}`;
          promises.push(
            engine.withLock(key, async () => {
              return j;
            })
          );
        }
  ...
  describe('Suite 2: Single-Key High-Contention Benchmark', () => {
    it('test_02_1: 100 concurrent workers competing for single key verify monotonic fencing tokens', async () => {
      const numWorkers = 20;
      ...
      const workerFn = async (engine: StorageMutexEngine) => {
        for (let i = 0; i < iterationsPerWorker; i++) {
          const res = await engine.withLock('hot_key', async (lock) => {
